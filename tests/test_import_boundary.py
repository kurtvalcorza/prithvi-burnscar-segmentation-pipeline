...
from prithvi_burnscar_segmentation_pipeline import validate_dataset, validate_inputs

PACKAGE = Path(__file__).resolve().parents[1] / "src" / "prithvi_burnscar_segmentation_pipeline"


def test_package_import_does_not_import_model_libraries(forbid_model_imports):
    import importlib

    import prithvi_burnscar_segmentation_pipeline

    importlib.reload(prithvi_burnscar_segmentation_pipeline)
