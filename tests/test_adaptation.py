...
from prithvi_burnscar_segmentation_pipeline import ADAPTATION_MODES, PrithviBurnScarPipeline  # noqa: E402
from prithvi_burnscar_segmentation_pipeline import pipeline as pl  # noqa: E402
...
def _pipeline() -> PrithviBurnScarPipeline:
    return PrithviBurnScarPipeline(model=_StubModel(), device="cpu", weights_dir=Path("unused"), source="stub")
...
def test_predict_and_evaluate_shapes():
    ...
    assert set(report["model"]) >= {"iou", "mean_iou", "f1", "accuracy"} and report["baseline_not_burned"]["recall"] == 0.0
